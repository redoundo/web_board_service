<script setup>
	...
	async function boardAxios() {
		const url = mapQueryString("/board", conditions.value);
		console.log(url);
	...
	async function checkUserByJwt() {
		...
		const success = function (value) {
			...
			if ((value ?? null) !== null && (value.data ?? null) !== null) {
				...
				router.push({ name: "boardWrite", query: conditions.value });
			}
		};
...
<template>
	<div style="display: flex; flex-direction: column; align-items: center; margin-top: 10px">
		<div style="display: flex; flex-direction: column; width: 95%; min-width: 665px">
			<span style="margin-bottom: 0.75rem; font-size: 15px; text-align: left">자유 게시판</span>
			<div>
				<SearchOptions
					v-bind:categories="categories"
					v-bind:conditions="conditions"
					v-on:changeCondition="changeCondition"
					v-on:emitAxios="boardAxios" />
