<script setup>
	...
	function goto(id) {
		router.push({ name: "boardView", query: { contentId: id } });
	}
	// TODO : comment 반환 여부 확인되지 않음. 제목 말 줄임 구현 안함.
</script>
<template>
	<div
		style="
		...
		<div style="width: 90%">
			<span
				style="
					...
					width: 100%;
				">
				<span style="flex-basis: 80%; font-weight: bold">자유 게시판</span>
				<span style="flex-basis: 20%; font-size: 0.75rem">
					<button style="color: white" type="button" v-on:click="router.push({ name: 'board' })">
						더보기 +
					</button>
				</span>
			</span>
			...
			<div style="width: 100%" v-if="(props.content ?? null) !== null && props.content.length > 0">
				<span
					...
					<!--        <button class="goToBoardDetailBtn" type="button" v-on:click="goto(item['boardId'])">-->
					<!--          {{item['title']}}-->
					<!--        </button>-->
						...
						<span
							...
							<span
								style="
									...
									gap: 5px;
								">
								<span style="flex-basis: 70%">
									<router-link
										class="titleLink"
										v-bind:to="{ name: 'boardView', query: { contentId: item['boardId'] } }">
										{{ item["title"] }}
									</router-link>
								...
								<span style="flex-basis: 20%; text-align: center" v-if="item['fileExistence']">
			...
			<div style="padding: 5px; padding-top: 10px" v-else>게시글이 존재하지 않습니다.</div>
		</div>
	</div>
</template>
