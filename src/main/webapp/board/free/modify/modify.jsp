...
<script type="text/javascript">
    //request가 초기화 되었으므로 쿼리스트링을 통해 한번만 다시 받아온다.
    function adjustAjax() {
        $.ajax({
...
<div id="mgt_course_modify">
    ...
    <div class="OptForm">
        ...
        <div>
            ...
            <div>
                <input class="input apply" onsubmit="alertAjax()" form="ModifyForm" type="submit" formaction="<%=request.getRequestURL().toString().replace("http://localhost:8080" , "").replace("modify.jsp" , "updateModify.jsp")%>"
...
<script type="text/javascript">
    ...
    function alertAjax () {
        ...
        const originalPassword = $("#originalPassword").attr("value");
        //submit 여부를 통제하는 변수
        //TODO : onSubmit에서 false가 반환되면 submit이 되지 않는지 여부 확인이 필요하다.
        let valid = false;
        $.ajax({
            type : "POST" , url : window.location.href.replace("http://localhost:8080" , "").replace("modify.jsp" , "encryptChecking.jsp") ,
